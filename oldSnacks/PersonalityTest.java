...
Take a breath. Look inward
	...
	public static String getName() {
		...
		while(true) {
			...
			while(true) {
				...
				entry = sc.next().trim();
				try {
					//int confirmName = Integer.parseInt(entry);
	...
	public static void navigateMenu(String name) {
		while(true) {
			System.out.printf("""
-------------------BEFORE YOU BEGIN-------------------
%s, I can walk you through why this matters, 
...
--> 
				...
				case "2": 
					System.out.println();
					System.out.println("Look into The Mirror... honestly");
					break;
	...
	public static ArrayList<String> getQuestions (String name) {
		ArrayList<ArrayList<String>> xtics = new ArrayList<>();
		ArrayList<ArrayList<String>> countAB = new ArrayList<>();
	  ...
	  "B: latitude, freedom"
		...
		xtics.add(new ArrayList<>());
		xtics.add(new ArrayList<>());
		xtics.add(new ArrayList<>());		
		xtics.add(new ArrayList<>());
		
		countAB.add(new ArrayList<>());
		countAB.add(new ArrayList<>());
		countAB.add(new ArrayList<>());		
		countAB.add(new ArrayList<>());
		
		for(int i = 0; i < 20; i++) {
			System.out.println(questionsA[i]);
			System.out.println(questionsB[i]);
			System.out.println();
			...
			if(choice.equals("A")) {
				xtics.get(rem).add(questionsA[i]);				
			} else {
				xtics.get(rem).add(questionsB[i]);
			}
			countAB.get(rem).add(choice.toUpperCase());
		}
	
		System.out.printf("Hello %s, your selections:", name);	
		System.out.println();
		
		for(int j = 0; j < xtics.size(); j++) {
			int count = 0;
			for(int k = 0; k < xtics.get(j).size(); k++) {
				System.out.printf("%n%s", xtics.get(j).get(k));			
			}
			
			int countA = 0;
			int countB = 0;
			for(String letter: countAB.get(j)) {
				if(letter.equals("A")) countA++;
				else if(letter.equals("B")) countB++;
			}
			
			
			String ch = (countA > countB) ? "A" : "B";
			personality.add(ch);
			
			System.out.printf("%nNumber of selected As: %s%nNumber of selected Bs: %s", countA, countB);
			System.out.println();
		}
		
		if(personality.get(0).equals("A")) personality.set(0, "E");
		else personality.set(0, "I");
				
		if(personality.get(1).equals("A")) personality.set(1, "S");
		else personality.set(1, "N");
		
		if(personality.get(2).equals("A")) personality.set(2, "T");
		else personality.set(2, "F");
		
		if(personality.get(3).equals("A")) personality.set(3, "J");
		else personality.set(3, "P");
				
		return personality;	
	}
	
		public static void showResult(ArrayList<String> personality) {
		String message = "";
		
		switch(String.join("", personality)) {
			case "ISTJ": 
				message = """
...
The Mirror reveals your edge — refined, bold, and always forward.	
	...
	public static void main(String[] args) {	
		while(true) {
			...
			navigateMenu(name);
			ArrayList<String> personality = getQuestions(name);
			System.out.println();
			
			while(true) {
				System.out.println("Are you now ready to see yourself? (yes/no)\n--> ");
